---
title: Docker ビルド
description: Dockerfile のレイヤー、エントリポイントのオーケストレーション、環境変数、およびローカルでの使用方法。
sidebar:
  order: 4
i18n:
...
ライブ開発サーバーワークフローを希望するコンテンツ執筆者は、[コンテンツ執筆者向けローカルプレビュー](../content-authors/)を参照してください。
...
:::note
`astro.config.mjs` と `content.config.ts` はテーマパッケージから提供されるもので、このリポジトリではバージョン管理されていません。開発サーバーを実行する前に `node_modules` からコピーする必要があります。Docker ビルドではエントリポイントがこれを自動的に処理します。
:::

:::note
`src/content/docs/` はイメージビルド時に空にされ、gitignore に登録されています。テストコンテンツは手動で配置してください。コミットされることはありません。
:::
...
イメージのビルドと公開を行うワークフローについては、[CI/CD とガバナンス](../ci-cd/)を参照してください。
