...
文档构建器是一个容器化的 [Astro](https://astro.build/) + [Starlight](https://starlight.astro.build/) 系统。内容仓库提供 Markdown/MDX 文件；构建器提供框架、主题和构建逻辑。两者在构建时于 Docker 容器内汇合。
...
这意味着构建器仓库本身附带的 `src/content/docs/` 目录是空的。内容仅在构建时出现。
...
这使得配置保持集中管理——内容仓库和构建器本身都不需要维护自己的 Astro 配置。
...
src/
  ...
  lib/
    placeholder-store.ts         # 状态管理：加载、保存、计算、发射
  scripts/
...
`src/content.config.ts` 使用 Starlight 的加载器和 schema 定义了一个 `docs` 集合：
...
任何放置在 `src/content/docs/` 中的 `.md` 或 `.mdx` 文件都会成为构建站点中的一个页面。
