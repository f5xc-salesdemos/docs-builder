...
构建器支持 [Mermaid](https://mermaid.js.org/) 图表，采用两阶段处理：remark 插件在构建时准备标记，客户端渲染器生成 SVG。
...
| 输出结构 | `<div class="mermaid-container">`，其 `data-mermaid-src` 属性包含转义后的源代码 |
| 回退内容 | `<pre class="mermaid">` 包含原始源代码（在 JS 渲染前可见） |
...
1. 从 `data-mermaid-src` 读取原始图表源代码
2. 对源代码执行占位符替换（见下文）
...
在渲染之前，图表源代码会经过与 DOM 遍历器相同的 `substituteText()` 函数处理（遍历器机制详见[占位符系统](../placeholder-system/)）：
...
这意味着像 `xCUSTOMER_ASNx` 这样的占位符标记可以在 Mermaid 图表定义中使用。当用户在表单中更改值时，`placeholder-change` 事件会触发使用更新后的值对所有图表进行完整的重新渲染。

## 错误处理

如果 `mermaid.render()` 抛出异常（例如，由于图表源代码中的语法错误），catch 块会直接在容器中显示错误信息：
...
remark 插件在构建时将其转换为容器 div。客户端将其渲染为替换了占位符值的 SVG。
