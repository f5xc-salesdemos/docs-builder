...
Copies `astro.config.mjs` and `content.config.ts` from the theme package. The theme is the single source of truth for Astro configuration — see [Architecture](../architecture/) for the theme system design.
...
For content authors who want a live dev server workflow, see [Local Preview for Content Authors](../content-authors/).
...
See [CI/CD and Governance](../ci-cd/) for the workflow that builds and publishes the image.
