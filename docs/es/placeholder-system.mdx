...
description: >-
  ...
  cómo añadir nuevos marcadores de posición.
sidebar:
  order: 2
i18n:
...
El sistema de marcadores de posición permite a los lectores personalizar direcciones IP, ASNs y otros valores específicos de implementación a lo largo de la documentación. Los autores escriben tokens en su Markdown; el navegador los reemplaza con valores proporcionados por el usuario en tiempo de ejecución.

## Formato de Token

Los tokens siguen el patrón de expresión regular:
...
|-------|-----------|-------------|
| `type` | sí | `"text"` para entrada de texto libre, `"dropdown"` para menús desplegables |
...
`src/lib/placeholder-store.ts` gestiona todo el estado de los marcadores de posición.
...
| Función | Propósito |
|---------|-----------|
| `getDefaults()` | Devuelve un mapa de cada clave de marcador de posición a su valor `default` del JSON |
| `loadValues()` | Lee de localStorage, recurre a `getDefaults()` como respaldo |
...
`getAllValues()` combina los valores ingresados por el usuario con los valores calculados, proporcionando un mapa completo para la sustitución.
...
Este evento impulsa tanto las actualizaciones de los spans del DOM como el re-renderizado de Mermaid.
...
- **Renderizado**: Itera `FIELD_GROUPS`, renderizando un `<fieldset>` por grupo. Cada clave obtiene un `<input>` (tipo texto) o un `<select>` (tipo desplegable)
...
`src/scripts/placeholder-dom.ts` gestiona el reemplazo de tokens del lado del cliente.
...
3. Para cada nodo de texto que coincide con la expresión regular del token, lo divide en un fragmento de documento con nodos de texto plano y elementos `<span data-ph="KEY" class="ph-value">`
...
| `astro:page-load` | `init` | Recorre nuevamente el DOM después de la navegación del lado del cliente de Astro |

## Cómo: Añadir un Nuevo Marcador de Posición

1. **Añadir la entrada JSON** en `src/data/placeholders.json`:
   ```json
   "MY_NEW_VALUE": {
     "type": "text",
     "default": "example",
...
2. **Añadir la clave a un grupo de campos** en `src/lib/placeholder-store.ts`. Ya sea añadiéndola al array `keys` de un grupo existente o creando un nuevo grupo en `FIELD_GROUPS`.
...
## Cómo: Añadir un Valor Calculado

Los valores calculados se derivan de otros marcadores de posición. Para añadir uno:

1. Añada una tabla de búsqueda (si es necesario) en `src/lib/placeholder-store.ts`, siguiendo el patrón de `cidrToMask`.
...
2. Extienda `getComputedValues()` para incluir la nueva clave derivada:
   ...
   export function getComputedValues(values: Record<string, string>): Record<string, string> {
     ...
     return {
       ...
       MY_COMPUTED: derivedValue,  // añadir aquí
     };
   }
   ```
