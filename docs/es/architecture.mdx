...
description: >-
  ...
  compilación para el constructor de documentación.
sidebar:
  order: 5
i18n:
...
El constructor de documentación es un sistema containerizado basado en [Astro](https://astro.build/) + [Starlight](https://starlight.astro.build/). Los repositorios de contenido proporcionan archivos Markdown/MDX; el constructor suministra el framework, el tema y la lógica de compilación. Las dos mitades se encuentran en tiempo de compilación dentro de un contenedor Docker.
...
Esto significa que el repositorio del constructor se distribuye con un directorio `src/content/docs/` vacío. El contenido solo aparece en tiempo de compilación.

## Sistema de temas

El tema de Starlight se publica como un paquete npm separado. `package.json` utiliza un alias de npm para mapear un nombre corto al paquete del registro con scope:
...
Esto mantiene la configuración centralizada — los repositorios de contenido y el propio constructor no mantienen su propia configuración de Astro.
...
src/
  components/
    ...
    PlaceholderFormWrapper.astro # Wrapper de Astro, carga script DOM y CSS global
  content/
  ...
  scripts/
    placeholder-dom.ts           # Walker DOM del lado del cliente y renderizador Mermaid
```

## Colección de contenido

`src/content.config.ts` define una única colección `docs` usando el loader y esquema de Starlight:
