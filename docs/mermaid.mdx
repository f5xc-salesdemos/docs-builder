...
Before rendering, the diagram source passes through the same `substituteText()` function used by the DOM walker (see [Placeholder System](../placeholder-system/) for the walker mechanism):
