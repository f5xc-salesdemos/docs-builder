---
title: Mermaid 다이어그램
description: 'Remark 플러그인, 클라이언트 사이드 CDN 렌더링, 다이어그램 내 플레이스홀더 치환, 오류 처리.'
sidebar:
  order: 3
i18n:
...
빌더는 2단계 처리를 통해 [Mermaid](https://mermaid.js.org/) 다이어그램을 지원합니다: 빌드 시 remark 플러그인이 마크업을 준비하고, 클라이언트 사이드 렌더러가 SVG를 생성합니다.

## Remark 플러그인

remark-mermaid 플러그인(`docs-theme` npm 패키지에서 제공)은 Astro 빌드 중에 실행됩니다. `unist-util-visit`을 사용하여 `lang === 'mermaid'`인 펜스 코드 블록을 찾아 HTML로 대체합니다:
...
| HTML 엔티티 이스케이핑 | `&`, `<`, `>`, `"` — `data-mermaid-src` 내 속성 주입을 방지 |
| 출력 구조 | 이스케이핑된 소스를 담고 있는 `data-mermaid-src` 속성이 포함된 `<div class="mermaid-container">` |
| 폴백 콘텐츠 | 원본 소스가 포함된 `<pre class="mermaid">` (JS가 렌더링하기 전까지 표시됨) |
...
Mermaid는 번들에 포함되지 않고 CDN에서 온디맨드로 로드됩니다:
...
`startOnLoad: false`는 Mermaid가 페이지를 자동으로 스캔하는 것을 방지합니다. `securityLevel: 'loose'`는 다이어그램 내 클릭 이벤트와 링크를 허용합니다.
...
2. 소스에 플레이스홀더 치환을 실행합니다 (아래 참조)
3. 컨테이너를 비우고 `data-processed` 속성을 제거합니다
4. 랜덤 ID로 `mermaid.render()`를 호출하여 SVG를 생성합니다
...
`mermaid.render()`가 예외를 발생시키면 (예: 다이어그램 소스의 구문 오류로 인해), catch 블록이 컨테이너에 직접 오류를 표시합니다:
...
이를 통해 페이지의 나머지 부분을 중단시키지 않으면서 작성 오류를 확인할 수 있습니다.
...
`mermaid` 언어 태그를 사용하여 표준 펜스 코드 블록을 작성합니다:
...
remark 플러그인이 빌드 시 이를 컨테이너 div로 변환합니다. 클라이언트는 플레이스홀더 값이 치환된 SVG로 이를 렌더링합니다.
