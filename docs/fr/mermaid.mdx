...
description: >-
  Plugin remark, rendu côté client via CDN, substitution de placeholders dans
  les diagrammes et gestion des erreurs.
sidebar:
  order: 3
i18n:
...
## Substitution de placeholders dans les diagrammes

Avant le rendu, la source du diagramme passe par la même fonction `substituteText()` utilisée par le parcoureur DOM (voir [Système de placeholders](../placeholder-system/) pour le mécanisme de parcours) :
...
|-------------|-----------|-----------------|
| Changement de valeur de placeholder | `placeholder-change` | `handleChange()` appelle `renderMermaidDiagrams()` avec les nouvelles valeurs |
