...
Der Docs-Builder ist ein containerisiertes [Astro](https://astro.build/) + [Starlight](https://starlight.astro.build/)-System. Content-Repos stellen Markdown/MDX-Dateien bereit; der Builder liefert das Framework, das Theme und die Build-Logik. Die beiden Hälften treffen zur Build-Zeit innerhalb eines Docker-Containers aufeinander.
...
Das Einstiegspunkt-Skript (`docker/entrypoint.sh`) führt die Kopie durch:
...
Das Starlight-Theme wird als separates npm-Paket veröffentlicht. Die `package.json` verwendet einen npm-Alias, um einen Kurznamen auf das Paket in der Scoped Registry abzubilden:

```json
"@f5xc-salesdemos/docs-theme": "^1.29.0"
```

Das Theme-Paket besitzt die `astro.config.mjs`. Zur Build-Zeit kopiert das Einstiegspunkt-Skript sie in das Projektstammverzeichnis:
...
Jede `.md`- oder `.mdx`-Datei, die in `src/content/docs/` abgelegt wird, wird zu einer Seite in der gebauten Website.
