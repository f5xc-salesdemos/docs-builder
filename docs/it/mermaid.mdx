...
description: >-
  Plugin remark, rendering lato client tramite CDN, sostituzione dei segnaposto
  nei diagrammi e gestione degli errori.
sidebar:
  order: 3
i18n:
...
| Contenuto di fallback | `<pre class="mermaid">` con il sorgente grezzo (visibile fino al rendering JS) |
...
Mermaid viene caricato on demand da un CDN — non è incluso nel bundle:
...
Prima del rendering, il sorgente del diagramma passa attraverso la stessa funzione `substituteText()` utilizzata dal DOM walker (vedi [Sistema di Segnaposto](../placeholder-system/) per il meccanismo del walker):
...
Se `mermaid.render()` genera un'eccezione (ad esempio, a causa di un errore di sintassi nel sorgente del diagramma), il blocco catch mostra l'errore direttamente nel container:
...
Questo rende gli errori di authoring visibili senza compromettere il resto della pagina.
...
|---------|--------|-------------|
| Cambio del valore dei segnaposto | `placeholder-change` | `handleChange()` chiama `renderMermaidDiagrams()` con i nuovi valori |
| Navigazione di pagina Astro | `astro:page-load` | `init()` chiama `renderMermaidDiagrams()` per la nuova pagina |
