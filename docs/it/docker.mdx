...
`docker/Dockerfile` utilizza una build multi-step basata su `node:24-alpine`:
...
| Layer | Cosa | Perché è memorizzato separatamente nella cache |
|-------|------|----------------------|
| 1 | `COPY package*.json` + `npm ci` | Le dipendenze cambiano raramente; memorizzare questo layer nella cache evita di riscaricarle ad ogni build |
| 2 | `COPY . .` | Codice sorgente del framework |
| 3 | `rm -rf src/content/docs/*` | Rimuove qualsiasi contenuto segnaposto in modo che appaia solo il contenuto iniettato |
| 4 | `COPY entrypoint.sh` | Script di orchestrazione della build |

## Entrypoint

`docker/entrypoint.sh` viene eseguito ogni volta che il container si avvia. Esegue i seguenti passaggi in ordine:
...
Copia `astro.config.mjs` e `content.config.ts` dal pacchetto del tema. Il tema è l'unica fonte di verità per la configurazione di Astro — consultare [Architettura](../architecture/) per il design del sistema del tema.
...
Copia il contenuto montato nella directory della content collection di Astro. Termina con un errore se la directory del contenuto è mancante.
...
Se `DOCS_TITLE` non è impostato tramite variabile d'ambiente, lo estrae dal campo `title:` del frontmatter di `index.mdx`.
...
Copia il sito generato nel punto di mount dell'output.
...
| `OUTPUT_DIR` | `/output` | Percorso dove viene copiato il sito generato |
...
| `GENERATE_PDF` | `false` | Abilita la generazione PDF dopo la build |
| `PDF_FILENAME` | `docs` | Nome del file di output per il PDF generato |
| `LLMS_OPTIONAL_LINKS` | `[]` | Array JSON di URL di siti figli per la sezione Optional di `llms.txt` |

## Utilizzo Locale

Per generare la documentazione localmente utilizzando Docker:
...
Questo monta la directory locale `docs/` come contenuto e scrive il sito generato in `output/`.

Per gli autori di contenuti che desiderano un flusso di lavoro con dev server live, consultare [Anteprima Locale per Autori di Contenuti](../content-authors/).
...
Aprire `http://localhost:4321`. L'Hot Module Replacement (HMR) funziona per le modifiche a componenti e plugin.

:::note
`astro.config.mjs` e `content.config.ts` provengono dal pacchetto del tema e non sono versionati in questo repository. È necessario copiarli da `node_modules` prima di avviare il dev server. L'entrypoint gestisce questo automaticamente nelle build Docker.
:::

:::note
`src/content/docs/` viene svuotata al momento della build dell'immagine ed è presente nel gitignore. Inserire manualmente il contenuto di test — non verrà committato.
:::
...
Due tag vengono pubblicati ad ogni build:

| Tag | Descrizione |
|-----|-------------|
| `latest` | Build più recente dal branch `main` |
| `<sha>` | SHA completo del commit Git per build riproducibili |

Consultare [CI/CD e Governance](../ci-cd/) per il workflow che genera e pubblica l'immagine.
