...
建構器透過兩階段處理支援 [Mermaid](https://mermaid.js.org/) 圖表：remark 外掛在建構時準備標記，客戶端渲染器則產生 SVG。
...
在渲染之前，圖表原始碼會通過 DOM 遍歷器使用的相同 `substituteText()` 函式進行處理（遍歷器機制請參閱[佔位符系統](../placeholder-system/)）：
