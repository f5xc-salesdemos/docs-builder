...
function walkTextNodes(root: Node, values: Record<string, string>) {
	...
	while ((node = walker.nextNode())) {
		if (
			...
			PH_REGEX.test(node.textContent || "")
		) {
		...
		while ((m = PH_REGEX.exec(original)) !== null) {
			if (m.index > lastIndex) {
				fragment.appendChild(
					document.createTextNode(original.slice(lastIndex, m.index)),
				);
			}
...
function updateSpans(values: Record<string, string>) {
	document
		.querySelectorAll<HTMLSpanElement>("span[data-ph]")
		.forEach((span) => {
